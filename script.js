...
function renderProducts(){
 ...
 game.products.forEach(p=>{
  ...
  if(!p.unlocked){div.innerHTML=`<b>${p.name}</b><br>Wird freigeschaltet ab Ruf ${p.unlockReputation}`; box.appendChild(div); return;}
  let s=priceState(p);
...
function upgradeProduct(id){
 ...
 game.money-=cost;
 p.level++; 
 p.sell*=1.2; // teurer und attraktiver
 p.exp+=10;
 ui();
}
...
function renderStaff(){
 ...
 game.staff.forEach(s=>{
  ...
  <button onclick="upgradeStaff(${s.id})">Skillen (${200*s.level}€)</button>
  <button class="danger" onclick="fireStaff(${s.id})">Kündigen</button>`;
  b.appendChild(div);
 });
}
window.upgradeStaff=id=>{let s=game.staff.find(x=>x.id==id); let cost=200*s.level; if(game.money<cost) return; game.money-=cost; s.level++; s.service++; s.sales++; s.logistics++; ui();}
window.fireStaff=id=>{let i=game.staff.findIndex(x=>x.id==id); if(i>=0) game.staff.splice(i,1); ui();}

// ---------------- KUNDEN & VERKAUF ----------------
function autoSell(){
 let baseCustomers=3 + Math.floor(game.reputation/5) + game.products.filter(p=>p.unlocked).length;
 let discountBoost=game.products.reduce((s,p)=>s+p.discount,0)/50;
 game.customers=Math.floor(baseCustomers*(1+discountBoost)+game.day*0.3);
 game.products.forEach(p=>{
  if(!p.unlocked||p.stock<=0) return;
  let demand=game.customers*(1+(p.discount/100));
  let state=priceState(p);
  if(state==="yellow") demand*=0.8;
  if(state==="red") demand*=0.5;
  let sold=Math.min(Math.floor(demand),p.stock);
  if(sold<=0) return;
...
function unlockByReputation(){
 game.products.forEach(p=>{if(!p.unlocked && game.reputation>=p.unlockReputation){p.unlocked=true; p.stock=10; game.report.push(`🔓 ${p.name} jetzt verfügbar`);}})
}
...
function nextDay(){
 ...
 game.expenses+=staffCost; game.money-=staffCost;
 let profit=game.income-game.expenses;
 if(profit>0) game.reputation+=0.5; if(profit<0) game.reputation-=0.3;
 ui();
}

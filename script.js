...
{id:3,name:"Apfel",buy:0.8,sell:10,stock:10,unlock:0},
...
div.innerHTML=`
<b>${s.name}</b>
<button onclick="autoOrder(${s.id})">Automatisch nachbestellen</button>
`;
box.appendChild(div);
});
}

function autoOrder(supplierId){
...
cost+=supplier.fee;
if(game.money<cost) {log("❌ Nicht genug Geld für Lieferant"); return;}
game.money-=cost;
log(`📦 Automatisch nachbestellt von ${supplier.name} (${cost}€)`);
ui();
}
...
// ================= WARNINGS =================
function warning(){
log("⚠️ Kunden beschweren sich über hohe Preise!");
if(Math.random()<0.3){
log("⚖️ Abmahnung – 500€ Strafe");
game.money-=500;
}
}
...
if(game.autoReorder) game.suppliers.forEach(s=>autoOrder(s.id));

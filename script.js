...
takeoverThreat:0,
prestige:0,
unlock:{management:false,staff:false,sustain:false,politics:false,branches:false,acquisition:false,hostile:false,prestige:false,trade:false},
statsLog:[],
currentTrade:null
};
...
if(game.marketShare>=40) unlock("tradePanel","trade");
...
// Handelspartnerchance
if(game.unlock.trade && Math.random()<0.015) offerTrade();
...
// ================= TRADE OFFERS =================
function offerTrade(){
let cashGain=1000+Math.floor(Math.random()*2000);
let marketGain=2+Math.floor(Math.random()*5);
let reputationLoss=1+Math.floor(Math.random()*3);
game.currentTrade={cash:cashGain,market:marketGain,risk:reputationLoss};
const box=el("tradeOffer");
box.innerHTML=`🤝 Handelspartner bietet: +${cashGain}€ Cash, +${marketGain}% Marktanteil, Risiko -${reputationLoss} Ruf
<button onclick="acceptTrade()">Annehmen</button> <button onclick="declineTrade()">Ablehnen</button>`;
}

function acceptTrade(){
if(!game.currentTrade) return;
game.cash+=game.currentTrade.cash;
game.marketShare+=game.currentTrade.market;
game.reputation-=game.currentTrade.risk;
log("🤝 Handelspartner angenommen");
game.currentTrade=null;
renderTradeOffer();
recalc();ui();
}

function declineTrade(){game.currentTrade=null;renderTradeOffer();log("✖ Handelspartner abgelehnt");}
function renderTradeOffer(){
el("tradeOffer").innerHTML=game.currentTrade?el("tradeOffer").innerHTML:"";
}

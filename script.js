...
function renderProducts(){
 ...
 game.products.forEach(p=>{
  ...
  if(!p.unlocked){
   div.innerHTML=`<b>${p.name}</b><br>Wird freigeschaltet ab Ruf ${p.unlockReputation}`;
   box.appendChild(div);
   return;
  }
  let s=priceState(p);
  ...
  div.innerHTML=`
   ...
   EK ${p.buy}€ | VK <input type="number" value="${p.sell}" data-id="${p.id}">
 ...
 document.querySelectorAll("[data-id]").forEach(i=>{
  i.oninput=()=>{let p=game.products.find(x=>x.id==i.dataset.id); p.sell=parseFloat(i.value); saveGame();}
 });
 document.querySelectorAll("[data-discount]").forEach(i=>{
  i.oninput=()=>{let p=game.products.find(x=>x.id==i.dataset.discount); p.discount=parseFloat(i.value)||0; saveGame();}
 });
}

// ---------------- AUTO ORDER ----------------
el("autoOrder").onchange=e=>{game.autoOrder=e.target.checked; saveGame();}
el("orderAmount").onchange=e=>{game.orderAmount=parseInt(e.target.value); saveGame();}
el("reorderLimit").onchange=e=>{game.reorderLimit=parseInt(e.target.value); saveGame();}
...
window.upgradeStaff=id=>{
 ...
 game.money-=cost; s.level++; s.service++; s.sales++; s.logistics++; ui();
};
...
function autoSell(){
 game.products.forEach(p=>{
  ...
  let s=priceState(p);
  if(s==="yellow") demand*=0.8;
  if(s==="red") demand*=0.5;
  demand*=1+(p.discount/100);
  let sold=Math.min(p.stock,Math.floor(demand));
  if(sold<=0) return;
  p.stock-=sold;
...
function animateProduct(id){
 ...
 divs.forEach(d=>{
  if(d.querySelector(`input[data-id="${id}"]`)){

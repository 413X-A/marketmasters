...
function calculateCustomers() {
 ...
 return Math.floor(baseCustomers*(1+discountBoost+staffBoost)+dayBoost);
}

function autoSell() {
 game.customers = calculateCustomers();
 let totalMoney = game.customers*50; // jede Kunde hat 50€ Kaufkraft
 ...
 game.products.forEach(p=>{
  ...
  game.income += revenue;
  game.xp += affordable;
  game.report.push(`🛒 ${p.name}: ${affordable} verkauft (${Math.floor(revenue)}€)`);
  animateProduct(p.id);
 });
}
...
function nextDay(){
 ...
 if(game.income - game.expenses>0) game.reputation+=0.5;
 else game.reputation-=0.3;
